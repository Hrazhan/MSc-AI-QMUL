...
import players.groupAU.evo.Individual;
import players.groupAU.evo.Mutation;
import players.groupAU.hybrids.MCTSNode;
import players.groupAU.utils.FMBudget;
import players.groupAU.utils.RHEAParamsGroupAU;
import players.groupAU.utils.Utilities;
...
public class GameInterface {
    ...
    GameInterface(RHEAParamsGroupAU params, Random random, int playerID) {
        this.params = params;
        this.random = random;
...
//        System.out.println("Update value: " + update_value);
    ...
    private Types.ACTIONS navieBayesModel(GameState gs) {
        StateHeuristic rootStateHeuristic;
        long [][][] actionCount = {
                {
                        {1, -2, 3},
                        {2, 3, 4}
        ...
        float [] nextactionprob = new float[6];
        int actionIndex = 0;

        for(int i = 0; i<6; i++) {
            CustomHeuristic stateEvaluation = new CustomHeuristic(gs);
            int playerId = gs.getPlayerId();
            rootStateHeuristic = new CustomHeuristic(gs);
            double state = rootStateHeuristic.evaluateState(gs.copy());

            float probablity_of_action_given_state = getProbabilityOfactiongivenState(actionCount, state, i);
            float probability_of_actioon = getProbabilityOfaction(actionCount, i);

            nextactionprob[i] = (probability_of_actioon*probablity_of_action_given_state);
        }
        float max = -1;
        for (int i = 0; i < 6; i++) {
            System.out.print(nextactionprob[i] + " ");
            if (nextactionprob[i] > max) {
                max = nextactionprob[i];
                actionIndex = i;
            }
        }
        System.out.println("===========> max = "+max+" actionIndex = "+actionIndex);
        return Types.ACTIONS.all().get(actionIndex);
    }

    public static float getProbabilityOfactiongivenState(long [][][]actionCount, double state, int action) {
        float pro;
        long count_of_action = 0;
        long total = 0;
        for(long [][] tupelSet: actionCount) {
            for (long [] set: tupelSet) {
                if (set[2]==state && set[1]==action)
                    count_of_action++;
                if (set[2]==state)
                    total++;
            }
        }
        pro = count_of_action/total;
        return pro;
    }

    public static float getProbabilityOfaction(long [][][] actionCount, int action) {
        int total = actionCount.length;
        int count = 0;
        for(long [][] tupelSet: actionCount) {
            for (long [] set: tupelSet) {
                if (set[1]==action)
    ...
    private boolean isEnemyAdjacentToUs(
...
//
//                ArrayList<Vector2d> itemsTempList = items.get(type);
//                if(itemsTempList == null) {
//                    itemsTempList = new ArrayList<>();
//                }
//                itemsTempList.add(position);
//                items.put(type, itemsTempList);
//
...
//                }
//
//                if(Types.TILETYPE.getAgentTypes().contains(type) &&
...
//            }
//        }
//        if(isEnemyAdjacentToUs(items, dist, enemies)){
//            System.out.println("Enemy Bomb Action");
...
//        int actionIdx = random.nextInt(nActions);
        ...
        actions.put(3, 15.0);
        actions.put(4, 15.0);
        actions.put(5, 35.0); // bomb action have higher probability than other actions to be chosen randomly
        int actionIdx = choice(actions);
