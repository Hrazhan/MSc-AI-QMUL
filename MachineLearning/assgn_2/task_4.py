...
# X_phonemes_1_2 = ...
X_phonemes_1 = X_full[phoneme_id == 1, :]
X_phonemes_2 = X_full[phoneme_id == 2, :]
X_phonemes_1_2 = np.vstack([X_phonemes_1, X_phonemes_2])
########################################/
...
########################################/
# f1_array = np.arange(min_f1, max_f1, 1)
# f2_array = np.arange(min_f2, max_f2, 1)

f1_array = range(min_f1, max_f1)
f2_array = range(min_f2, max_f2)
...
for i, f1_val in enumerate(f1_array):
    for j, f2_val in enumerate(f2_array):
        grid[i][j] = [f1_val , f2_val]
...
for i in range(N_f2):
    ...
    pred_1 = get_predictions(phoneme_1_3["mu"], phoneme_1_3["s"], phoneme_1_3["p"], grid[:, i]) 
    pred_1_3.append(np.sum(pred_1, axis=1))

    pred_2 = get_predictions(phoneme_2_3["mu"], phoneme_2_3["s"], phoneme_2_3["p"], grid[:, i])
    pred_2_3.append(np.sum(pred_2, axis=1))


# print(len(pred_1_3))
# print(len(pred_1_3[0]))

M = np.ndarray(shape=(N_f2, N_f1))

for i in range(len(pred_1_3)):
    for j in range(len(pred_1_3[0])):
        if pred_1_3[i][j] > pred_2_3[i][j]:
            M[i][j] = 0.0
        else:
            M[i][j] = 1.0
